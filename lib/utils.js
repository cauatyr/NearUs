...
// Horario semanal por defecto: Lun–Sáb 09:00–19:00, Dom cerrado, sin pausa.
...
export function minutosDeHora(t) {
  const [h, m] = String(t).split(':').map(Number)
  return h * 60 + m
}

// ¿La pausa (almuerzo) de un día es usable? Tiene que estar completa y dentro
// del horario de ese día; si no, se ignora (mejor abrir de más que romper).
export function pausaValida(franja) {
  if (!franja || !franja.pausa) return false
  const { inicio, fin } = franja.pausa
  if (!inicio || !fin) return false
  const ini = minutosDeHora(inicio)
  const f = minutosDeHora(fin)
  if (ini >= f) return false
  return ini >= minutosDeHora(franja.apertura) && f <= minutosDeHora(franja.cierre)
}

// Texto de la(s) franja(s) de un día: "10:00–20:00" o, con pausa de almuerzo,
// "10:00–13:30 y 14:30–20:00".
export function textoFranja(franja) {
  if (!franja || !franja.abierto) return 'Cerrado'
  if (!pausaValida(franja)) return `${franja.apertura}–${franja.cierre}`
  return `${franja.apertura}–${franja.pausa.inicio} y ${franja.pausa.fin}–${franja.cierre}`
}
...
export function resumenHorario(horarioSemanal) {
  ...
  // Comparamos el texto completo del día (incluye la pausa de almuerzo), así
  // dos días con la misma apertura/cierre pero distinta pausa NO se agrupan.
  const mismoHorario = abiertos.every((d) => textoFranja(d) === textoFranja(abiertos[0]))

  if (mismoHorario) {
    const franja = textoFranja(abiertos[0])
  ...
  // Horarios distintos por día → agrupamos los días CONSECUTIVOS que comparten
  // el mismo texto (ej. "Lun–Vie 10:00–13:30 y 14:30–20:00 · Sáb 09:00–18:00").
  const porDia = new Map(abiertos.map((d) => [DIAS_SEMANA.indexOf(d.dia), d]))
  const grupos = []
  for (const i of indices) {
    const texto = textoFranja(porDia.get(i))
    const ultimoGrupo = grupos[grupos.length - 1]
    if (ultimoGrupo && ultimoGrupo.texto === texto && ultimoGrupo.fin === i - 1) {
      ultimoGrupo.fin = i
    } else {
      grupos.push({ ini: i, fin: i, texto })
    }
  }
  // Demasiados tramos → el resumen dejaría de caber en el card.
  if (grupos.length > 3) return `${primero}–${ultimo} · horario variable`

  return grupos
    .map((g) => {
      const dias = g.ini === g.fin ? DIAS_SEMANA[g.ini] : `${DIAS_SEMANA[g.ini]}–${DIAS_SEMANA[g.fin]}`
      return `${dias} ${g.texto}`
    })
    .join(' · ')
}
...
// `pausa` = { inicio, fin } (almuerzo): se descartan los inicios que caen dentro
// de la pausa Y los que harían que el servicio la invada.
export function horariosDeFranja(apertura, cierre, duracion = 0, paso = 30, pausa = null) {
  if (!apertura || !cierre) return []
  const ini = minutosDeHora(apertura)
  const fin = minutosDeHora(cierre)
  const limite = duracion > 0 ? fin - duracion : fin - 1
  const dur = duracion > 0 ? duracion : 1
  const pIni = pausa && pausa.inicio ? minutosDeHora(pausa.inicio) : null
  const pFin = pausa && pausa.fin ? minutosDeHora(pausa.fin) : null
  const chocaConPausa = (t) => pIni !== null && pFin !== null && t < pFin && t + dur > pIni

  const out = []
  for (let t = ini; t <= limite; t += paso) {
    if (chocaConPausa(t)) continue
    out.push(`${String(Math.floor(t / 60)).padStart(2, '0')}:${String(t % 60).padStart(2, '0')}`)
  }
  return out
}

// ¿El negocio está abierto AHORA según su horario_semanal (hora de Cuenca)?
// Respeta la pausa de almuerzo. Devuelve null si no hay horario configurado,
// para que el badge se oculte en vez de mentir.
export function horarioAbiertoDeSemanal(horarioSemanal) {
  if (!Array.isArray(horarioSemanal) || horarioSemanal.length === 0) return null
  const { fecha, minutos } = ahoraEnCuenca()
  const hoyCuenca = new Date(`${fecha}T00:00:00`)
  const franja = franjaDelDia(horarioSemanal, hoyCuenca)
  if (!franja) return null
  if (!franja.abierto) return false
  if (minutos < minutosDeHora(franja.apertura) || minutos >= minutosDeHora(franja.cierre)) return false
  if (pausaValida(franja)) {
    const ini = minutosDeHora(franja.pausa.inicio)
    const fin = minutosDeHora(franja.pausa.fin)
    if (minutos >= ini && minutos < fin) return false
  }
  return true
}
...
// Comisión que NearUs descuenta de cada servicio COBRADO POR LA PLATAFORMA
// (metodoPago === 'inapp'). Los pagos en el local no pasan por NearUs.
// 10% durante el lanzamiento — se ajusta acá, en un solo lugar.
export const COMISION_NEARUS = 0.10

export function comisionDeReservas(reservas) {
  const cobradoEnApp = reservas
    .filter((r) => r.estado !== 'cancelada' && r.metodoPago === 'inapp')
    .reduce((s, r) => s + Number(r.precio || 0), 0)
  const comision = cobradoEnApp * COMISION_NEARUS
  return { cobradoEnApp, comision, netoNegocios: cobradoEnApp - comision }
}
