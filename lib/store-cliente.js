...
function aCliente(user) {
  ...
  return {
    ...
    celular: m.celular || '',
    foto: m.foto || null,
    terminosVersion: m.terminos_version || null,
    terminosEn: m.terminos_en || null
  }
}
...
export const useCliente = create((set, get) => ({
  ...
  // Edita nombre / celular / foto (guardados en user_metadata). La foto es un
  // dataURL comprimido chico (avatar) — suficiente para el MVP sin bucket aparte.
  async actualizarPerfil({ nombre, celular, foto }) {
    const payload = {}
    if (nombre !== undefined) payload.nombre = nombre
    if (celular !== undefined) payload.celular = celular
  ...
  // Deja constancia de qué versión de los términos aceptó esta cuenta. Se guarda
  // en user_metadata para que valga en cualquier dispositivo; el navegador
  // guarda su propia copia para el visitante que todavía no tiene cuenta.
  async aceptarTerminos(version) {
    if (!get().cliente) return { error: null }
    const { data, error } = await supabase.auth.updateUser({
      data: { terminos_version: version, terminos_en: new Date().toISOString() }
    })
    if (error) return { error: traducir(error.message) }
    set({ cliente: aCliente(data.user) })
    return { error: null }
