...
export const useDatosStore = create((set, get) => ({
  ...
  async cargar() {
    ...
    try {
      const [neg, ser, emp, res] = await Promise.all([
      ...
      const primerError = [neg, ser, emp, res].find((r) => r.error)
  ...
  async agregarNegocio({ nombre, categoria, descripcion, direccion, barrio, telefono, lat, lng, horario, logo, email, password }) {
  ...
  // ===== SERVICIOS =====
  async agregarServicio({ negocioId, nombre, duracion, precio, categoria }) {
    const id = generarId('s')
    const payload = { id, negocio_id: negocioId, nombre, duracion, precio, categoria }
  ...
  // ===== EMPLEADOS =====
  async agregarEmpleado({ negocioId, nombre, cargo, avatar, celular, diasTrabajo }) {
    const id = generarId('e')
    const payload = { id, negocio_id: negocioId, nombre, cargo, avatar, celular, dias_trabajo: diasTrabajo }
