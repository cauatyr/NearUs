    ...
    <main className="min-h-screen bg-zinc-50 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10">
        <div className="w-full max-w-md">
          <h1 className="text-3xl font-semibold text-zinc-900 text-center">Acceder al panel</h1>
          ...
          <button
            ...
            className="w-full bg-amber-100 hover:bg-amber-200 text-amber-800 border border-amber-200 rounded-2xl py-3.5 px-4 font-semibold flex items-center justify-center gap-2 transition"
          >
      ...
      >
        <div className="p-5 border-b border-zinc-100 flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold text-zinc-900">Elige el negocio</h2>
            <p className="text-xs text-zinc-500 mt-0.5">
              Entrarás al panel como si fueras el dueño de ese negocio.
            </p>
          </div>
          <button onClick={onCerrar} className="w-8 h-8 grid place-items-center rounded-xl hover:bg-zinc-100 text-zinc-500 shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>

        <div className="p-3 border-b border-zinc-100">
          <div className="bg-zinc-50 rounded-xl px-3 py-2 flex items-center gap-2">
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          {filtrados.length === 0 && (
            <p className="text-center text-sm text-zinc-500 py-10">Ningún negocio coincide.</p>
          )}
          {filtrados.map((n) => {
            ...
            return (
              <button
                key={n.id}
                onClick={() => elegir(n.id)}
                className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-zinc-50 text-left transition"
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm text-zinc-900 truncate">{n.nombre}</div>
